...
use ssh_broker_proto::sshbroker::v1::{credential_service_server::CredentialServiceServer, session_service_server::SessionServiceServer};
...
async fn main() -> Result<()> {
    ...
    let limits = Limits {
        max_exec_output_bytes: cfg.max_exec_output_bytes,
        max_shell_output_bytes: cfg.max_shell_output_bytes,
        max_shell_seconds: cfg.max_shell_seconds,
        max_scp_bytes: cfg.max_scp_bytes,
    };
    let rate_limiter = if cfg.max_requests_per_minute == 0 {
        RateLimiter::disabled()
    } else {
        RateLimiter::new(cfg.max_requests_per_minute)
    };
    let audit = AuditLogger::new(cfg.audit_path(), cfg.audit_log_commands);

    let cred_svc = CredentialSvc::new(store.clone(), rate_limiter.clone(), audit.clone());
    let sess_svc = SessionSvc::new(store, sessions, known_hosts_path, limits, rate_limiter, audit);
...
#[cfg(unix)]
async fn serve_uds(uds_path: PathBuf, cred_svc: CredentialSvc, sess_svc: SessionSvc) -> Result<()> {
...
async fn serve_tcp(addr: SocketAddr, cfg: Config, cred_svc: CredentialSvc, sess_svc: SessionSvc) -> Result<()> {
